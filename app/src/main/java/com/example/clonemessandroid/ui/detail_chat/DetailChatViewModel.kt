...
//    var liveListChat :MutableLiveData<List<ChatDetailModel>> = MutableLiveData()
    var liveListChat :MutableLiveData<ChatModel> = MutableLiveData()
    var livedataImgUpdaload :MutableLiveData<ResponseBody> = MutableLiveData()
    @SuppressLint("CheckResult")
    fun getListDetailChat(idChat:String,page:Int){
        detailChatApi.getListChatApi(idChat,page)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({it->
              liveListChat.value  =it
            },{it->
